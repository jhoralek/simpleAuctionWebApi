...
const actions: ActionTree<AuctionState, RootState> = {
  ...
  getDetail({ commit, rootState, dispatch }, id: number): Promise<boolean> {
    return new Promise<boolean>(resolve => {
      return axios
        .get(`${rootState.settings.apiUrl}/auctions/getForEdit?id=${id}`, {
  ...
  create({ commit, rootState, dispatch }, auction: Auction): Promise<boolean> {
    return new Promise<boolean>(resolve => {
      return axios
        .post(`${rootState.settings.apiUrl}/auctions/create`, auction, {
          headers: { authorization: rootState.auth.token }
        })
        .then(response => {
  ...
  getAllForAdmin({ commit, rootState, dispatch }): Promise<boolean> {
    return new Promise<boolean>(resolve => {
      return axios
        .get(`${rootState.settings.apiUrl}/auctions/getAllForAdmin`, {
          headers: { authorization: rootState.auth.token }
        })
        .then(response => {
          commit(
            AUCTION_CHANGE_TABLE_STATE,
            response.data as AuctionTableDto[]
          );
          return resolve(true);
        })
        .catch(error => {
  ...
  update({ commit, rootState, dispatch }, auction: Auction): Promise<boolean> {
    return new Promise<boolean>(resolve => {
      return axios
        .put(`${rootState.settings.apiUrl}/auctions`, auction, {
          headers: { authorization: rootState.auth.token }
        })
        .then(response => {
  ...
  delete({ commit, rootState, dispatch }, auctionId: number): Promise<boolean> {
    return new Promise<boolean>(resolve => {
      return axios
        .delete(`${rootState.settings.apiUrl}/auctions/delete`, {
          params: {
            id: auctionId
          },
          headers: {
  ...
  getLookup({ commit, rootState, dispatch }): Promise<boolean> {
    return new Promise<boolean>(resolve => {
      return axios
        .get(`${rootState.settings.apiUrl}/auctions/getAllActiveLookup`, {
          headers: { authorization: rootState.auth.token }
        })
        .then(response => {
          commit(
            AUCTION_CHANGE_LOOKUP_STATE,
            response.data as AuctionLookupDto[]
          );
          return resolve(true);
        })
        .catch(error => {
  ...
  getAllLookup({ commit, rootState, dispatch }): Promise<boolean> {
    return new Promise<boolean>(resolve => {
      return axios
        .get(`${rootState.settings.apiUrl}/auctions/getAllLookup`, {
          headers: { authorization: rootState.auth.token }
        })
        .then(response => {
          commit(
  ...
  getFutureAutions({ commit, rootState, dispatch }): Promise<number> {
    return new Promise<number>(resolve => {
      return axios
        .get(`${rootState.settings.apiUrl}/auctions/getAllInFeature`)
        .then(response => {
